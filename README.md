...
- **Plex remote-access check** — Plex isn't a container, so it's reached from the WSL2 host via the Windows host IP (same trick `plex-proxy` uses). Checks `/identity` for basic reachability and `myPlexMappingState` (the field Plex's own Settings → Remote Access page reads) for whether remote access is actually mapped. Auto-remediates a stuck mapping by toggling `PublishServerOnPlexOnlineKey` off/on via the prefs API — same effect as unchecking/rechecking "Enable Remote Access," without restarting the Windows service or dropping active streams. Requires `PLEX_TOKEN` in `.env`; same 15-minute cooldown as the gluetun check
